...
describe('runtime indexing', async () => {
  ...
  it('poll: returns status with correct shape', async () => {
    const result = (await fetch('/__ai-ready/poll', { method: 'POST', headers: authHeaders })) as {
      indexed: number
      remaining: number
      duration: number
      complete: boolean
  ...
  it('poll: caps limit at 50', async () => {
    ...
    const result = (await fetch('/__ai-ready/poll?limit=100', { method: 'POST', headers: authHeaders })) as { indexed: number }
    expect(result.indexed).toBeLessThanOrEqual(50)
  })

  it('poll: all=true processes multiple pages', async () => {
    const result = (await fetch('/__ai-ready/poll?all=true&timeout=5000', { method: 'POST', headers: authHeaders })) as { indexed: number, complete: boolean }
    expect(typeof result.indexed).toBe('number')
    expect(typeof result.complete).toBe('boolean')
  })

  it('status: returns indexing status', async () => {
    const result = (await fetch('/__ai-ready/status', { headers: authHeaders })) as { total: number, indexed: number, pending: number }
  ...
  it('prune endpoint: removes stale routes', async () => {
    ...
    const result = (await fetch('/__ai-ready/prune?ttl=604800', { method: 'POST', headers: authHeaders })) as { pruned: number, ttl: number, dry: boolean }
  ...
  it('restore: endpoint exists and responds', async () => {
    ...
    const result = await fetch('/__ai-ready/restore', { method: 'POST', headers: authHeaders }).catch((e: { statusCode: number }) => ({ error: true, statusCode: e.statusCode })) as { error?: boolean, statusCode?: number, restored?: number, cleared?: boolean }
