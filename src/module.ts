...
`)
        ...
        // Cloudflare Pages: Nitro auto-excludes every prerendered HTML route
        // from `_routes.json` so CF serves them as static assets. That bypasses
        // the worker, and with it our content-negotiation middleware — Accept:
        // text/markdown on a prerendered route returns the cached HTML. Strip
        ...
        if (config.runtimeContentNegotiation && (cfPreset === 'cloudflare-pages' || cfPreset === 'cloudflare_pages')) {
          ...
          if (routesExists) {
            const routes = JSON.parse(await readFile(routesPath, 'utf-8')) as { include?: string[], exclude?: string[] }
            const before = routes.exclude?.length ?? 0
            // Keep entries that are wildcards (e.g. /_nuxt/*) or have a file
            // extension (e.g. /favicon.ico, /sitemap.xml, /llms.txt, /foo.md).
            // Drop entries with no extension — those are HTML route shells that
            // Nitro generated by stripping `.html` from prerender output.
            routes.exclude = (routes.exclude || []).filter((entry) => {
              if (entry.endsWith('*'))
                return true
              const lastSeg = entry.split('/').pop() || ''
              return lastSeg.includes('.')
            })
            const removed = before - routes.exclude.length
            if (removed > 0) {
              logger.info(`Routing ${removed} prerendered HTML route(s) through worker for content negotiation`)
