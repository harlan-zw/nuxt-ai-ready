...
Publish MCP discovery metadata at `/.well-known/mcp/server-card.json`{lang="bash"} when MCP Toolkit runs on the server. Set this option to `false` to disable the route.

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `title` | `string` | Site or Toolkit name | Human readable server title |
| `description` | `string` | Toolkit or site description | Server description |
| `instructions` | `string` | Toolkit instructions | Instructions for clients |
| `iconUrl` | `string` | First Toolkit icon | Absolute HTTP or HTTPS icon URL |
| `documentationUrl` | `string` | None | Absolute HTTP or HTTPS documentation URL |
| `cacheMaxAge` | `number` | `3600` | Browser and shared cache lifetime in seconds |

The card route is omitted when Toolkit is absent, disabled or unavailable in a static deployment.
