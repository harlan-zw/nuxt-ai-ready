...
## `webmcp: boolean | { siteTools?: boolean, maxOutputChars?: number, searchLimit?: number, exposedTo?: string[] }`{lang="ts"}

- Default: `false`{lang="ts"}

Register [WebMCP](/docs/ai-ready/guides/webmcp) tools with the AI agent running in the browser via `document.modelContext`{lang="ts"}.

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `siteTools` | `boolean` | `true` | Register the built-in `list_pages`, `search_pages` and `get_page_markdown` tools |
| `maxOutputChars` | `number` | `1500` | Characters a single tool response may return before truncation |
| `searchLimit` | `number` | `10` | Results returned by `search_pages` by default |
| `exposedTo` | `string[]` | – | Secure origins allowed to discover the tools. Same-origin only by default |
...
Enabling this auto-imports `useWebMcpTool()`{lang="ts"}. Set `siteTools: false`{lang="ts"} to register your own tools without the built-in ones or the `/__ai-ready/pages`{lang="bash"} endpoint.
