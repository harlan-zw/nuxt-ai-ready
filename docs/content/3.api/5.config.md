...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `siteTools` | `boolean \| SiteToolName[]` | `true` | Register all, none or selected built-in tools |
| `maxOutputChars` | `number` | `1500` | Positive integer character budget for one built-in response |
| `searchLimit` | `number` | `10` | Default `search_pages` result count, from 1 to 50 |
...
`SiteToolName`{lang="ts"} is `list_pages`{lang="ts"}, `search_pages`{lang="ts"} or `get_page_markdown`{lang="ts"}. Invalid numeric values produce a build warning and use the default.

Nuxt auto-imports `useWebMcpTool()`{lang="ts"} and `useWebMcpSupported()`{lang="ts"} when you enable WebMCP. Set `siteTools: false`{lang="ts"} or `[]`{lang="ts"} to register your own tools without adding the built-in tools or the `/__ai-ready/pages`{lang="bash"} endpoint.
