...
Enable scheduled cron task that runs every minute. When enabled, automatically enables `runtimeSync` for background indexing and runs IndexNow sync if `indexNow` is configured.
