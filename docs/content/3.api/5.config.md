...
| `exposedTo` | `string[]` | None | Trusted origins allowed to discover the tools. Same-origin only by default |
...
Nuxt auto-imports `useWebMcpTool()`{lang="ts"} when you enable WebMCP. Set `siteTools: false`{lang="ts"} to register your own tools without adding the built-in tools or the `/__ai-ready/pages`{lang="bash"} endpoint.
