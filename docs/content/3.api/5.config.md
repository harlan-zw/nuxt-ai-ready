...
Publish an [Agent Skills Discovery v0.2.0 index](/docs/ai-ready/guides/agent-skills). Skills in `skills/<name>/SKILL.md`{lang="text"} are discovered in the project and its layers, served at `/.well-known/agent-skills/<name>/SKILL.md`{lang="text"}, `/skills/<name>/SKILL.md`{lang="text"}, and (for a single skill or the one named in `root`{lang="ts"}) `/SKILL.md`{lang="text"}, and listed in llms.txt. Nothing is published when no skill exists. `dir`{lang="ts"} changes the folder or turns discovery off, `skills`{lang="ts"} adds explicit local or external entries, `llmsTxt: false`{lang="ts"} leaves llms.txt alone, and `false`{lang="ts"} turns the feature off. The `ai-ready:agent-skills`{lang="ts"} hook sees every entry before publication.
...
Generate a Markdown sitemap at `/sitemap.md`{lang="bash"} from indexed pages. Pages are grouped by top-level path segment under `##`{lang="ts"} headings, and each link points at the page's `.md` representation. When a page records a last update, it becomes the link's title text.

The same flag appends a `## Sitemap`{lang="ts"} section to every Markdown page, linking back to `/sitemap.md`{lang="bash"}, as the Vercel agent-readability audit expects.
...
Advertise llms.txt with `rel="describedby"`{lang="ts"} on HTML and Markdown responses, as the llms.txt v2 spec requires. HTML pages get a `<link rel="describedby" href="/llms.txt">`{lang="html"} tag, and both variants get a `Link`{lang="http"} header entry.
