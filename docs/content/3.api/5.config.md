...
Publish current SEP-2127 discovery metadata at `<mcp-route>/server-card`{lang="bash"} when MCP Toolkit runs on the server. For the default Toolkit route, the card is available at `/mcp/server-card`{lang="bash"}. Set this option to `false` to disable the route.
