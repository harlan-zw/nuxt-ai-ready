...
- Default: Enabled, unless the project has an ISR route rule
...
The module disables negotiation automatically when it finds an ISR route rule. ISR caches responses by URL, so a negotiated redirect can otherwise replace the cached HTML response. Set `false`{lang="ts"} for another URL-based deployment cache, or `true`{lang="ts"} to override ISR detection.
