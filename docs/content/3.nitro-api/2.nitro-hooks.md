...
Called when a page is indexed at runtime (requires `runtimeIndexing.enabled`). Use this to sync with external systems like vector databases or search indexes.
...
| `title` | `string` | Page title |
| `description` | `string` | Page description |
| `headings` | `string` | JSON-stringified headings array |
