...
Relative `anchor`{lang="ts"} and `href`{lang="ts"} values resolve against the deployed site URL. With `app.baseURL: '/docs/'`{lang="ts"}, Nuxt redirects the origin-root well-known request to the base-aware handler and resolves `/api`{lang="ts"} as `https://example.com/docs/api`{lang="http"}. Absolute URLs remain unchanged, so one catalog can describe APIs on other origins.

## Entries and Relations

An entry uses its `anchor`{lang="ts"} as the API endpoint or link context. It must contain at least one relation target. Targets accept either one object or an array.
