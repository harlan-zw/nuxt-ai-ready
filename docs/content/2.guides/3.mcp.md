...
Runtime MCP servers publish a card at `/.well-known/mcp/server-card.json`{lang="bash"}. The card uses the protocol version from your installed MCP SDK and the identity, route and capabilities exposed by MCP Toolkit.

```bash
curl https://example.com/.well-known/mcp/server-card.json
```
