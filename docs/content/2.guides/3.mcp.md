...
The card publishes static identity and connection metadata, including the protocol version from the installed MCP SDK. Tools, resources, prompts and capabilities remain discoverable through the live MCP protocol. Responses include CORS headers, a strong `ETag`{lang="http"}, and support `If-None-Match`{lang="http"} revalidation.

Set `aiReady.mcpServerCard`{lang="ts"} to `false` to remove the card. A fully static `nuxi generate`{lang="bash"} deployment does not publish one because it cannot host an MCP server.
