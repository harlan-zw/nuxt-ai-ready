...
Nuxt AI Ready tracks when page content was last updated by hashing markdown content and comparing it across builds. AI agents can use these timestamps to assess content freshness.
...
Pages with timestamps in meta tags use those values:
...
Timestamps work correctly across deployments only when the content hash manifest persists between builds. By default, the manifest is stored at `node_modules/.cache/nuxt-seo/ai-index/content-hashes.json`.

**Critical**: Without persistent storage, every build treats all pages as new, assigning fresh timestamps each time.
...
Search engines see accurate content freshness dates without manual configuration.
