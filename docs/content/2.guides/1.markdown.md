...
Negotiated redirects include `Cache-Control: private, no-store`{lang="http"} and a complete `Vary`{lang="http"} header. This prevents a compliant shared cache from storing the redirect under the HTML URL. A cache that bypasses the origin before the middleware runs can still return cached HTML to an AI client.

**Prerendered routes behind a CDN.** Request-time negotiation cannot run when a CDN serves the prerendered HTML asset directly. Configure the CDN to vary on all three headers, or use the discovery options below.
