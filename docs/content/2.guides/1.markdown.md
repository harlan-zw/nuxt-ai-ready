...
Nuxt AI Ready disables automatic negotiation when it finds an ISR route rule. For another deployment cache that stores responses by URL, disable it explicitly:
...
**Prerendered routes behind a CDN.** `Accept`{lang="http"} negotiation **does not work** on prerendered HTML routes
