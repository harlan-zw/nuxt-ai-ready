...
**Automatic syncing (cron).** When `cron: true`, IndexNow syncs automatically every minute after indexing runs:
...
**Manual sync via endpoint.** For manual integration, call the [sync endpoint](#sync-endpoint) directly instead of relying on cron.
...
| `Authorization` header | `Bearer <token>`{lang="html"} | - | Required if `runtimeSyncSecret` configured |
