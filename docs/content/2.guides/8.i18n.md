...
Locales an entry doesn't name keep the default locale's path, matching what i18n serves for an untranslated route. Entries that name no default locale at all fall back to prefixing the requested path, so spell out at least your default locale to keep alternates exact.
