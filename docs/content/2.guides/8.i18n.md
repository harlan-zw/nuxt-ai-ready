...
`/about` advertises `hreflang="fr"` as `/fr/a-propos`, and `/blog/hello` advertises `/fr/journal/hello`. Dynamic and catch-all segments carry their values across locales, and a page set to `false` for a locale gets no alternate for it.

Entries that don't name every locale fall back to prefixing the requested path, since the untranslated file path isn't recorded in the route table. Spell out each locale you serve to keep alternates exact.
