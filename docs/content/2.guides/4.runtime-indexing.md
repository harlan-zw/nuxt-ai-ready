...
On Node.js you must install [`better-sqlite3`](https://github.com/WiseLibs/better-sqlite3) yourself, since it's an optional peer dependency:
...
Runtime indexing is optional. If you only prerender your site, disable the database:
...
Build time indexing writes its own SQLite file. On Node 22 and later it uses `node:sqlite`.
If you build on Node 20, keep `better-sqlite3` installed.
...
`runtimeSync`, `cron` and `indexNow` write pages at runtime, so they need a database.
If you enable one of them, the build fails and names the option.
