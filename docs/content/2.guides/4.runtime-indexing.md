...
**IndexNow integration.** Combine runtime sync with [IndexNow](/docs/ai-ready/guides/indexnow) to notify search engines instantly when pages change. When you enable both `runtimeSync.cron` and `indexNow`, IndexNow syncs automatically after each scheduled indexing run.
...
The module uses [SQLite](https://sqlite.org) via [Drizzle ORM](https://orm.drizzle.team/) for page storage. It auto-detects the best driver for your platform:
...
Runtime indexing is optional. If you only prerender your site, the database stays disabled.
You do not need any config or runtime driver.

Set the option only when you want to force storage off:
...
Prerender still writes `llms.txt`, `llms-full.txt` and a `.md` twin for every page.

Build time indexing writes its own SQLite file. Node 22.13 and later use `node:sqlite`.
Earlier Node versions require `better-sqlite3` when build time indexing runs.

A disabled database turns off runtime page storage. You lose:

- The `list_pages`, `search_pages` and `get_page_markdown` tools.
- The MCP pages resource.
- `.md` conversion for routes you did not prerender.
...
`runtimeSync`, `cron` and `indexNow` write pages at runtime, so they enable storage.
If you force storage off, the build fails and names each conflicting option.
