...
**Cloudflare Pages**: Keep `cron: true` to register the production HTTP endpoint. Use an external scheduler to call `GET /__ai-ready/cron` with `Authorization: Bearer <token>`{lang="html"} header. See the [Cloudflare guide](/docs/ai-ready/guides/cloudflare#cloudflare-pages) for details.
::
