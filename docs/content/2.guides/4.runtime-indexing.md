...
You must provide the `secret` query param for POST endpoints if you configure `runtimeSync.secret`.
...
**Cloudflare Pages**: Does not support cron triggers. Use an external scheduler to call `GET /__ai-ready/cron?secret=<token>`{lang="html"}. See the [Cloudflare guide](/docs/ai-ready/guides/cloudflare#cloudflare-pages) for details.
::
