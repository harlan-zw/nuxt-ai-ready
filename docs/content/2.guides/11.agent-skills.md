...
Skill installers and people often expect a skill at the site root, such as `https://example.com/SKILL.md`{lang="text"}. Add `alias`{lang="ts"} to publish the same bytes there:
...
The alias must be a path-absolute route that ends in `.md`{lang="text"} and sits outside `/.well-known/`{lang="text"}. Content negotiation leaves the alias alone, so the response is the file itself, with no generated frontmatter and the same digest as the discovery artifact. The index keeps advertising the `.well-known`{lang="text"} URL.
