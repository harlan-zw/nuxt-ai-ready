...
Nuxt AI Ready generates `/llms.txt` during prerender following the [llms.txt proposal](https://llmstxt.org/). It also generates `/llms-full.txt` as a companion context export.
...
The formatter places link descriptions after the link on the same line. It puts section descriptions and `notes` in the heading-free preamble before the first H2. Links from every section marked `optional` are flattened into the proposal's single special `## Optional` file list.
